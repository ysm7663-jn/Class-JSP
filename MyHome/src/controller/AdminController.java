...
import command.admin.AdminPwUpdateCommand;
import command.admin.AdminSelectMemberDeleteCommand;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "/adminMemberDelete.admin":
				...
				pathNRedirect = command.execute(request, response);
				break;
			case "/adminSelectMemberDelete.admin":
				command = new AdminSelectMemberDeleteCommand();
				pathNRedirect = command.execute(request, response);
				break;
			case "/adminMemberUpdate.admin":
				...
				pathNRedirect = command.execute(request, response);
				break;
			case "/adminInsertMemberUpdate.admin":
				command = new AdminMemberUpdateCommand();
				pathNRedirect = command.execute(request, response);
				break;
			case "/adminPwUpdate.admin":
				command = new AdminPwUpdateCommand();
				pathNRedirect = command.execute(request, response);
				break;
			// 단순 이동
			case "/adminInsertMember.admin":
				pathNRedirect = new PathNRedirect();
				pathNRedirect.setPath("admin/adminInsertPage.jsp");
				pathNRedirect.setRedirect(false);
				break;
			case "/adminPwUpdatePage.admin":
				pathNRedirect = new PathNRedirect();
				pathNRedirect.setPath("admin/adminMemberPwUpdatePage.jsp");
				pathNRedirect.setRedirect(false);
				break;
