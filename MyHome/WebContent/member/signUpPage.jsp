...
<script>
	...
	function fn_idCheck() {
		...
		var regId = /^[a-z][0-9a-z]{4,19}$/;    // 정규식
		$('#mId').keyup(function(){ 		    // keyup() : 아이디를 입력할 때
			if( regId.test($('#mId').val()) ) { // 정규식 패턴을 만족한다는 의미
			$.ajax({
				url: '/MyHome/IdCheck',
				type: 'post',
				data: 'mId=' + $('#mId').val(),
				...
				success: function(responseObj) {
					if(responseObj.result) {
						$('#idCheckResult').text('사용할 수 있는 아이디입니다').css('color','limegreen');
					} else {
	...
	function fn_clear() {
		// each() : 각각 하나씩 하나하나를 $(this)라고 하나씩 부른다
		$('#clearBtn').click(function(){
			$('input[type="text"], input[type="password"]').each(function(){
				$(this).val('');
			});
			
			// input이 val() 쓰기 때문에 text()를 써야한다.
